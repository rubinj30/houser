...
export async function GET(request: Request, { params }: Context) {
  ...
  const { data: document } = await supabase.from("documents").select("storage_key").eq("id", documentId).maybeSingle();
  if (!document) return NextResponse.json({ error: "Document not found." }, { status: 404 });

  const { data, error } = await supabase.storage.from("documents").createSignedUrl(document.storage_key, 300);
  ...
  return NextResponse.redirect(`${data.signedUrl}${page && page > 0 ? `#page=${page}` : ""}`, 307);
}
