...
type ChatMessage = {
  ...
  actionResult?: { message: string; propertyId: string; workItemId: string; title: string };
  actionError?: string;
};
  ...
  const applyAction = async (messageId: string, action: HouserChatAction) => {
    ...
    try {
      ...
      setMessages((current) => current.map((message) => message.id === messageId ? {
        ...
        proposedAction: null,
        actionResult: { message: result.message, propertyId: result.workItem.propertyId, workItemId: result.workItem.id, title: result.workItem.title },
      } : message));
    } catch (cause) {
      setMessages((current) => current.map((message) => message.id === messageId ? {
        ...message,
      ...
      <section className="flex-1 space-y-5 py-5 sm:py-8" aria-live="polite" aria-label="Ask Houser conversation">
        {messages.map((message, index) => <article key={message.id} className={`flex gap-3 ${message.role === "user" ? "justify-end" : "justify-start"}`}>
          ...
          <div className={`max-w-[min(88%,44rem)] ${message.role === "user" ? "rounded-[22px] rounded-br-md bg-[var(--forest)] px-4 py-3 text-white" : "min-w-0"}`}>
            ...
            {message.actionResult ? <Link href={`/?property=${encodeURIComponent(message.actionResult.propertyId)}&work=${encodeURIComponent(message.actionResult.workItemId)}`} className="mt-3 flex items-center justify-between gap-3 rounded-[18px] border border-[var(--forest)]/15 bg-white p-3.5 text-[var(--forest)] surface-shadow"><div><p className="text-[10px] font-extrabold uppercase tracking-[0.12em]">Saved</p><p className="mt-1 text-xs font-extrabold text-[var(--ink)]">{message.actionResult.title}</p></div><ArrowRight className="size-4"/></Link> : null}
