...
function LinkedWorkDocuments({ workItemId, onAddDocument }: { workItemId?: string; onAddDocument: () => void }) {
  ...
  return <section className="rounded-[22px] border border-black/7 bg-white/65 p-4">
    <div className="flex items-center justify-between gap-3"><div className="flex min-w-0 items-center gap-2"><FileText className="size-4 shrink-0 text-[var(--forest)]"/><div><p className="text-[10px] font-extrabold uppercase tracking-[0.12em] text-[var(--muted)]">Documents</p><p className="mt-0.5 text-xs font-bold">{documents.length} {documents.length === 1 ? "attachment" : "attachments"}</p></div></div><button type="button" onClick={onAddDocument} className="flex min-h-10 shrink-0 items-center gap-1.5 rounded-xl border border-[var(--forest)]/15 bg-[var(--mint)]/45 px-3 text-xs font-extrabold text-[var(--forest)]"><Plus className="size-3.5"/> Add document</button></div>
    {isPending ? <p className="mt-3 text-xs font-bold text-[var(--muted)]">Loading documents…</p> : loadError ? <p role="alert" className="mt-3 text-xs font-bold text-[#8c3328]">{loadError}</p> : documents.length ? <div className="mt-3 space-y-2">{documents.map((document) => <a key={document.id} href={`/api/documents/${document.id}/view`} target="_blank" rel="noreferrer" className="flex min-h-14 items-center justify-between gap-3 rounded-xl border border-black/7 bg-white px-3 py-2.5 transition hover:border-[var(--forest)]/25 hover:bg-[var(--mint)]/20" aria-label={`Open original ${document.documentType}: ${document.filename}`}><div className="flex min-w-0 items-center gap-3"><div className="grid size-9 shrink-0 place-items-center rounded-lg bg-black/[0.035]"><FileText className="size-4 text-[var(--forest)]"/></div><div className="min-w-0"><p className="truncate text-xs font-extrabold">{document.filename}</p><p className="mt-1 text-[10px] capitalize text-[var(--muted)]">{document.documentType}{document.documentDate ? ` · ${formatDateOnly(document.documentDate)}` : ""}</p></div></div><span className="flex shrink-0 items-center gap-1 text-[10px] font-extrabold text-[var(--forest)]">Open original <ExternalLink className="size-3.5"/></span></a>)}</div> : <p className="mt-3 rounded-xl border border-dashed border-black/10 px-3 py-4 text-xs leading-5 text-[var(--muted)]">No quotes, invoices, or receipts attached yet. The inspection remains available in the evidence preview above.</p>}
  </section>;
}
...
type DocumentUploadType = "inspection" | "quote" | "invoice" | "receipt";
...
function DocumentUploadDialog({ propertyId, seed, findings, initialWorkItem, onClose }: { propertyId: string; seed: InspectionSeed; findings: Finding[]; initialWorkItem: Finding | null; onClose: () => void }) {
  ...
  return <div className="fixed inset-0 z-[60] grid items-end bg-[#0d1e17]/45 p-0 backdrop-blur-sm sm:place-items-center sm:p-6" role="presentation" onMouseDown={(event) => { if (!busy && event.currentTarget === event.target) onClose(); }}><section role="dialog" aria-modal="true" aria-labelledby="document-upload-title" className="max-h-[94dvh] w-full overflow-y-auto rounded-t-[28px] bg-[var(--paper)] p-5 shadow-2xl sm:max-w-xl sm:rounded-[28px] sm:p-7"><div className="flex items-start justify-between gap-4"><div><p className="text-[11px] font-extrabold uppercase tracking-[0.14em] text-[var(--forest)]">Private document import</p><h2 id="document-upload-title" className="font-display mt-1 text-2xl font-extrabold tracking-tight">{initialWorkItem ? "Attach document" : "Upload document"}</h2><p className="mt-2 text-sm leading-6 text-[var(--muted)]">Houser privately stores the original PDF and uses OpenAI to extract structured details for your review.</p>{initialWorkItem ? <p className="mt-3 rounded-xl bg-[var(--mint)]/55 px-3 py-2 text-xs font-bold text-[var(--forest)]">Attaching to: {initialWorkItem.title}</p> : null}</div><button type="button" onClick={onClose} disabled={busy} className="grid size-10 shrink-0 place-items-center rounded-xl bg-black/5 disabled:opacity-40" aria-label="Close"><X className="size-5"/></button></div>
    ...
    {phase === "review" && result && result.documentType !== "inspection" ? <FinancialDocumentUploadReview result={result} findings={findings} categories={[...new Set(seed.findings.map((item) => item.category))].sort()} areas={seed.areas} initialWorkItem={initialWorkItem} onClose={onClose} /> : null}
