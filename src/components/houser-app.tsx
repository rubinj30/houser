...
type DocumentUploadType = "inspection" | "quote" | "invoice";
type DocumentUploadResult =
  | (InspectionExtraction & { documentType: "inspection"; documentId: string; runId: string })
  | { documentType: "quote" | "invoice"; documentId: string; runId: string; normalized: NormalizedDocument };
...
function DocumentUploadDialog({ propertyId, onClose }: { propertyId: string; onClose: () => void }) {
  ...
  return <div className="fixed inset-0 z-50 grid items-end bg-[#0d1e17]/45 p-0 backdrop-blur-sm sm:place-items-center sm:p-6" role="presentation" onMouseDown={(event) => { if (!busy && event.currentTarget === event.target) onClose(); }}><section role="dialog" aria-modal="true" aria-labelledby="document-upload-title" className="max-h-[94dvh] w-full overflow-y-auto rounded-t-[28px] bg-[var(--paper)] p-5 shadow-2xl sm:max-w-xl sm:rounded-[28px] sm:p-7"><div className="flex items-start justify-between gap-4"><div><p className="text-[11px] font-extrabold uppercase tracking-[0.14em] text-[var(--forest)]">Private document import</p><h2 id="document-upload-title" className="font-display mt-1 text-2xl font-extrabold tracking-tight">Upload document</h2><p className="mt-2 text-sm leading-6 text-[var(--muted)]">Houser privately stores the original PDF and uses OpenAI to extract structured details for your review.</p></div><button type="button" onClick={onClose} disabled={busy} className="grid size-10 shrink-0 place-items-center rounded-xl bg-black/5 disabled:opacity-40" aria-label="Close"><X className="size-5"/></button></div>
